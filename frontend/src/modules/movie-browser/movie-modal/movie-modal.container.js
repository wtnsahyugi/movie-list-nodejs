...
import { closeMovieModal, openSeasonDetail } from './movie-modal.actions';
...
class MovieModalContainer extends React.Component {
  ...
  render() {
    const {isOpen, closeMovieModal, isLoading, openSeasonDetail} = this.props;
    ...
    const listSeason = (movie.seasons) ? movie.seasons.map((listValue, index) => {
        return (
          <tr key={index} onClick= {() => openSeasonDetail(movie.id ,listValue.id)}>
...
export default connect(
  ...
  { closeMovieModal, getMovieDetails, openSeasonDetail }
)(MovieModalContainer);
